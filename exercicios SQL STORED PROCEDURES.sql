...
--A primeira parte da stored procedure usa uma query com a cláusula WITH para criar uma tabela temporária chamada ranking. 
--Essa tabela temporária contém o nome do cliente, a quantidade de sinistros desse cliente e a posição do cliente no 
--ranking de quantidade de sinistros(atribuído através da função ROW_NUMBER).

--A segunda parte da stored procedure usa uma query que seleciona os N primeiros clientes com mais sinistros (dado pelo parâmetro @n),
--utilizando a tabela temporária ranking. A tabela resultante possui as colunas nome, posicao_ranking (posição no ranking)e qt_sinistros (quantidade de sinistros).

--A cláusula ORDER BY ordena os resultados pelo ranking em ordem crescente,
--para que o cliente com o maior número de sinistros seja exibido primeiro.

--Por fim, a stored procedure é finalizada com a palavra-chave END.
